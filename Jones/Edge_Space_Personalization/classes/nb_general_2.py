...
#                 print('T Here')
    ...
    if B:
        if graph:
            if ret_tau:
                return nx.DiGraph(S@T), nx.DiGraph(S@T-tau), nx.DiGraph(tau)
            return nx.DiGraph(S@T), nx.DiGraph(S@T-tau)
        if ret_tau:
            return S@T, S@T - tau, tau
        return S@T, S@T - tau
    if graph:
        if ret_tau:
            return nx.DiGraph(S@T), nx.DiGraph(tau)
        return nx.DiGraph(S@T)
    if ret_tau:
        return  S@T, tau
    return S@T
